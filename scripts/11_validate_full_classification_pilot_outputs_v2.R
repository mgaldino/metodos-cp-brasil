## 11_validate_full_classification_pilot_outputs_v2.R
## Valida JSONs dos tres subagentes do piloto v2 e consolida uma base CSV por agente.
...
paths <- list(
  ...
  issues = file.path(project_dir, "quality_reports", "full_classification_pilot_v2_validation_issues.csv"),
  summary = file.path(project_dir, "quality_reports", "full_classification_pilot_v2_validation_summary.md")
)
...
agents <- tibble(
  ...
  prompt_version = c(
    "agent_a_v2+common_schema_v2",
    "agent_b_v2+common_schema_v2",
    "agent_c_v2+common_schema_v2"
  )
)
...
for (agent_idx in seq_len(nrow(agents))) {
  ...
  for (row_idx in seq_len(nrow(manifest))) {
    ...
    purrr::walk(extra_envelope, function(field) {
      add_issue(agent_id, pid, rel_json_file, field, "extra_envelope_field", "ERROR",
                value = field, expected = paste(envelope_required, collapse = "; "))
    })
    ...
    purrr::walk(extra_fields, function(field) {
      add_issue(agent_id, pid, rel_json_file, field, "extra_classification_field", "ERROR",
...
summary_lines <- c(
  "# Validacao dos JSONs do piloto v2 de classificacao tripla",
  "",
  ...
  "- `data/processed/full_classification_pilot_v2/agent_a_classifications.csv`",
  "- `data/processed/full_classification_pilot_v2/agent_b_classifications.csv`",
  "- `data/processed/full_classification_pilot_v2/agent_c_classifications.csv`"
)
